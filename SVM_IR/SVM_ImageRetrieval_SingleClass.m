function SVM_ImageRetrieval_SingleClass()
...
ID=7;  %设置你要检索的图像类别ID(可任意设置成1-10的数,因为库中只有10类图像，每类100幅)

%1 构造训练与测试集
ImgNum=100;
N=50;   %设置训练图像的数量,就是从每类图像中选40幅用于SVM训练，另100-N幅用于测试
...
%负灰全加入测试集
TestFea=[TestFea; nFea(1:900,:)];
TestLabel=[TestLabel nLabel(1:900)];
TestImgFileName=[TestImgFileName nFN(1:900)];
...
% show_flg=1;  %因为是单类SVM了，SVM输出的是1或-1，则AUC无法统计了
...
for g=[0.01]
    for C=[100] %[0.0001 0.001 0.01 0.1 1]
        %合成命令
        Tcom1='svm\svm-train.exe -s 2 -t 2 -c ';
        Tcom2=[num2str(C) ' -g ' num2str(g)];
        Tcom3=' tmp\RSSvmTrain.scale tmp\分类模型.mdl';
        T=[Tcom1 Tcom2 Tcom3];
        %训练
        system(T);
        %system(['svm-train.exe -s 3 -t 2 -b 1 -c 100 -g 0.001 RSSvmTrain.scale 分类模型.mdl']);
        %预测
