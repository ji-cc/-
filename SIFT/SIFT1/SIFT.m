function Descriptors = SIFT(inputImage, Octaves, Scales, Sigma)
    ...
    for o = 1:Octaves
        ...
        tempM = zeros([row,col,Scales]); %记录梯度大小?
        for s = 1:Scales
   ...
   %% ---------------------------------------------取极值点----------------
    for o=1:Octaves
        ...
        for s=2:Scales-1
            ...
            for y=14:col-12
                for x=14:row-12
                    sub = images(x-1:x+1,y-1:y+1,s-1:s+1); % 获取上下两层子图像3*3大小?3*3澶у皬
                     % 中间点与周围26个点比较
                    if sub(2,2,2) > max([sub(1:13),sub(15:end)]) || sub(2,2,2) < min([sub(1:13),sub(15:end)])
                        ...
                        tempMagnitude = GradientMagnitudes(x-radius+a:x+radius-c,y-radius+b:y+radius-d,s).*weights(1+a:end-c,1+b:end-d); %璁＄畻鏋佸?肩偣闄勮繎鐨勬搴﹀ぇ灏?
